$(function () {
  d3.csv('data/network.csv', function(data){
    ...
    if('state' in parsed_url.searchKey){
      var state = state_name_hindi[parsed_url.searchKey.state.toUpperCase()]
    }
    else {
     var state = state_name_hindi['PUNJAB']
    }

    $('#subtitle').text(state)

    var states = [state_name_hindi['PUNJAB'], state_name_hindi['UP'], state_name_hindi['MANIPUR'], state_name_hindi['UTTARAKHAND'], state_name_hindi['GOA']];
    var states = _.without(states, state)
    _.each(states, function(state) { $("#otherstates").append('<p data-type="state">' + state + '</p>') })
    var agencies = ['NEWS18']
    _.each(agencies, function(agency) { $("#otheragency").append('<p data-type="agency">' + agency + '</p>') })

    $("#filter-state.dropdown span:first").text(state)
    $("#filter-agency.dropdown span:first").text(agency)
    data = _.filter(data, function(d){ return d.STATE == state_name_hindi_reverse[state] && d.AGENCY == agency })
  ...
  $('.gotopoll').on('click', function(){
    var _st = $("#filter-state.dropdown span:first").text()
    var _ag = $("#filter-agency.dropdown span:first").text()
    window.location = "/pollhindi?state="+state_name_hindi_reverse[_st]
  })
  ...
  $('body')
    ...
    .on("click", ".dropdown-content p", function(){
      ...
      var agency = $("#filter-agency span:last").text()
      var state = $(".dropdown span:first").text();

      d3.csv('data/network.csv', function(data){
        data = _.filter(data, function(d){ return d.AGENCY == agency && d.STATE == state_name_hindi_reverse[state] })
        if(data.length == 0){
          $('#nodata').show()
        }else{
          $('#nodata').hide()
...
function drawDonut(data){
  ...
  center_texts = center_texts == '2012' ? ['2012'] : ['बहुमत', raceto[state_name_hindi_reverse[$("#filter-state span:first").text()]]]
  ...
  // svg.append('g').attr('transform','translate(' + width/2 + ',' + ((height/2)+15) + ')').append('text').text($("#filter-agency span:first").text() == '2012' ? '2012' :'2017').attr('text-anchor','middle').attr('fill','black').style('font-size','50px').style('font-family', 'Helvetica Inserat LT Std')
  ...
  var path = party_arc.append("path")
      ...
      .attr("stroke", "black")
      .attr("fill", function(d,i){ return partyColors[d.data.PARTY] || defaultColor })
      .on('click', function(d){
      })
...
var initStackedBarChart = {
  draw: function(config) {
    ...
    var selected_state = state_name_hindi_reverse[$("#filter-state span:first").text()];
    var heading = d3.select('#'+domEle+' svg').append('g').attr('transform','translate(250,20)')
    heading.append('text')
            .text('कुल सीट - '+total_seats[selected_state])
    ...
    heading.append('text') // $("#filter-agency span:first").text()
            .text($("#filter-agency span:first").text() == '2012' ? 'नतीजे-2012' : 'GRAMENER की भविष्यवाणी')
            .style('fill', '#D2D2D1')
