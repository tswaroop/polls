...
function draw(state, year, curr_candidates, dataFilters){
  d3.csv('data/' + state_name_mapping[state] + '_agg.csv', function(data){
    ...
    // document.querySelector('#constFilterContainer').ontouchend = function() {  $("#constFilter").focus() };
    // document.body.ontouchend = function(e) {  e.preventDefault();$("#constFilter").focus() };
    // $("#constFilter").focus()
    // $('#constFilter').focusout(function(){
    //      $('#constFilter').focus();
    //  });
    ...
    _.each(dataFilters, function(filter){
      ...
      if(filter.column == 'PARTY'){
        ...
        var filtered = _.filter(data, function(d){
          return filter.type == 'ne' ? d[filter.column] != filter.value : d[filter.column].toUpperCase() == filter.value.toUpperCase()
        })
      }
    ...
    if(dataFilters.length > 0){
      ...
      if(state.toUpperCase() == 'MANIPUR'){
        var phase_filter_flag = _.includes(_.map(dataFilters,'column'), 'PHASE')
        over_all_flag = over_all_flag || phase_filter_flag
      }
...
function drawMap(state, topoJSONPath, data, curr_candidates, filtered_groups){
  ...
  d3.json(topoJSONPath, function(topodata){
    ...
    topoLayer.eachLayer(function(layer) {
      ...
      layer.setStyle({
        weight: currResolution > 1600 ? 2.5 : 0.5,
        fillColor: currResolution > 1600 ? '#6B6B6B' : '#D6C5A9',
        fillOpacity: 1,
        color: currResolution > 1600 ? 'black' : 'white',
      })
    ...
    _.each(latlngs, function(obj){
      ...
      var circle = new L.Constituency(
        ...
        {
          fillColor: currResolution > 1600 ? 'url(' + obj.partyColor + ')' : obj.partyColor,
          color: 'black',
          class: '.circle-shadow',
          weight: 2,
          fillOpacity: 1.0,
      ...
      circle.on('click', function(d){
        ...
        if(this.options.fillColor == 'black'){
          this.setStyle({
            fillColor: currResolution > 1600 ? 'url(' + d.target.options.data.color + ')' : d.target.options.data.color,
            color: 'black'
          })
          parsed_url.update({constituency: null})
          $("#constituency").hide();
          ...
          circles.forEach(function(circle) {
            if(circle.options.fillColor == 'black'){
              circle.setStyle({
                fillColor: currResolution > 1600 ? 'url(' + circle.options.data.color + ')' : circle.options.data.color,
                color: 'black'
              })
            }
          })
          this.setStyle({ fillColor: 'black', color: d.target.options.data.color })
          displayConstituency(d.target.options.data)
          parsed_url.update({constituency: d.target.options.data.area.toUpperCase() })
        }
        window.history.pushState('page2', '', parsed_url.protocol + '://' + parsed_url.origin + parsed_url.pathname + '?' + parsed_url.search);
      ...
      if(constituency){
        if(circle.options.data.area.toUpperCase() == constituency.toUpperCase()){
