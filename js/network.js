$(function () {
  d3.csv('data/network.csv', function(data){
    ...
    data = _.filter(data, function(d){ return d.STATE == state && d.AGENCY == agency })
    console.log(data.length, agency, state)
  ...
  $('.gotopoll').on('click', function(){
    ...
    window.location = "/poll?state="+_st
  })
  ...
  $('body')
    ...
    .on("click", ".dropdown-content p", function(){
      ...
      d3.csv('data/network.csv', function(data){
        data = _.filter(data, function(d){ return d.AGENCY == agency && d.STATE == state })
        if(data.length == 0){
          $('#nodata').show()
        }else{
          $('#nodata').hide()
...
var initStackedBarChart = {
  draw: function(config) {
    ...
    heading.append('text') // $("#filter-agency span:first").text()
            .text($("#filter-agency span:first").text() == '2012' ? 'Results-2012' : 'GRAMENER PROJECTIONS')
            .style('fill', '#D2D2D1')
