$(function () {
  d3.csv('data/2012.csv', function(data){
    ...
    $('.gotocomparison').on('click', function(){
      ...
      window.location = '/pollhindi?state='+state_name_hindi_reverse[_state]
    })

    var parsed_url = G.url.parse(window.location.href)
    if('agency' in parsed_url.searchKey){
      var agency = agency_alias_hindi[parsed_url.searchKey.agency.toUpperCase()]
    }
    else {
     var agency = agency_alias_hindi[$("#filter-agency span:first").text()];
    }
    ...
    _.each(agencies, function(agency) { $("#otheragency").append('<p data-type="agency">' + agency_alias_hindi[agency] + '</p>') })

    $("#filter-state.dropdown span:first").text(state)
    $("filter-agency.dropdown span:first").text(agency)
    data = _.filter(data, function(d){ return d.STATE == state_name_hindi_reverse[state] && d.AGENCY == agency_alias_hindi_reverse[agency] })
  ...
  $('body')
    ...
    .on("click", ".dropdown-content p", function(){
      ...
      d3.csv('data/temp.csv', function(data){
        data = _.filter(data, function(d){ return d.AGENCY == agency_alias_hindi_reverse[agency] && d.STATE == state_name_hindi_reverse[state] })
        if(data.length == 0){
          $('#nodata').show()
        }else{
          $('#nodata').hide()
...
var initStackedBarChart = {
  draw: function(config) {
    ...
    heading.append('text')
            .text($("#filter-agency span:first").text() == '2012' ? 'नतीजे - 2012' : $("#filter-agency span:first").text())
            .style('fill', '#D2D2D1')
